...
**Prefer rooting the anchor on the enclosing entity, then walking to the target.** When
an interactive surface represents a domain entity (an item, record, card, event), its
container usually carries a stable identifying attribute. Scope the anchor through that
and reach the placement slot structurally — this is far more durable than a clever
selector built only from the target widget's own (often generated or positional) markup.
Probe *upward* for such a hook before settling on a local anchor.
...
- **Brittle selectors:** hashed CSS-module class names (e.g.
  ...
  best → worst: **entity-semantic data attributes** (a `data-*` that names the underlying
  item/record — most durable, survives redesigns) → **ARIA / generic `data-*` presence**
  (`[aria-label]`, `[role]`, behavioural data hooks) → **stable structural `:has()` /
  child combinators** → **positional selectors (`:nth-*`, `:first-child`) and framework
  event/binding tokens** (locale-free but index- or build-dependent — they shift when
  markup or framework output changes) → **hashed CSS classes** (rotate most often). Reach
  for a positional or framework-token anchor only after confirming no entity-level data
  attribute exists on or above the target.
